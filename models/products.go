...
var ProductWhere = struct {
	...
	OfferID:   whereHelperint{field: "\"products\".\"offer_id\""},
	...
	Available: whereHelperbool{field: "\"products\".\"available\""},
	SellerID:  whereHelperint{field: "\"products\".\"seller_id\""},
}
...
var (
	productAllColumns            = []string{"offer_id", "name", "price", "quantity", "available", "seller_id"}
	productColumnsWithoutDefault = []string{"offer_id", "name", "price", "quantity", "available", "seller_id"}
	productColumnsWithDefault    = []string{}
	productPrimaryKeyColumns     = []string{"offer_id", "seller_id"}
...
func FindProduct(ctx context.Context, exec boil.ContextExecutor, offerID int, sellerID int, selectCols ...string) (*Product, error) {
	productObj := &Product{}
	...
	query := fmt.Sprintf(
		"select %s from \"products\" where \"offer_id\"=$1 AND \"seller_id\"=$2", sel,
	)

	q := queries.Raw(query, offerID, sellerID)
...
func (o *Product) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"products\" WHERE \"offer_id\"=$1 AND \"seller_id\"=$2"
...
func (o *Product) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindProduct(ctx, exec, o.OfferID, o.SellerID)
	if err != nil {
		return err
	}
...
func ProductExists(ctx context.Context, exec boil.ContextExecutor, offerID int, sellerID int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"products\" where \"offer_id\"=$1 AND \"seller_id\"=$2 limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, offerID, sellerID)
	}
	row := exec.QueryRowContext(ctx, sql, offerID, sellerID)
...
func (o *Product) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return ProductExists(ctx, exec, o.OfferID, o.SellerID)
}
